...
class MonkeyBusinessTest {
    ...
    private Map<Integer, Monkey> getInput(String inputFilePath) {
        ...
        for (String monkeyDefinition : inputFile.split("\r{0,1}\n\r{0,1}\n")) {
            final String[] lines = monkeyDefinition.split("\r{0,1}\n");
            final Monkey monkey = new Monkey.MonkeyBuilder()
                    ...
                    .div(Long.parseLong(lines[3].trim().split(" ")[3]))
    ...
    private Function<Long, Long> calc(String line, BiFunction<Long, Long, Long> operation) {
        final String a = line.trim().split(" ")[3];
        final String b = line.trim().split(" ")[5];
    ...
    private static Stream<Arguments> provideArgumentsForSecondTask() {
        return Stream.of(
                ...
                Arguments.of("src/test/resources/day11/input1.in", 27267163742L)
        );
    }
}